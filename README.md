...
- Ghost 쿨타임 15초
- 화면 밖에서 생성되어 직선으로 지나가는 미사일
- 미사일 충돌, 사망, 자동 리스폰
- 생존/근접 회피 점수
- 시간이 지날수록 증가하는 미사일 생성 속도와 동시 존재 수
...
- 미사일이 화면 밖에서 들어와 직선으로 지나가는가
- 미사일과 충돌하면 사망 후 자동 리스폰되는가
- Ghost 상태에서는 미사일 충돌을 무시하는가
- 시간이 지날수록 미사일 수와 생성 속도가 증가하는가
- 서버 재시작 후 재접속 가능한가

## 다음 작업 순서

멀티 접속 검증 이후에는 게임 감각을 다듬습니다.
